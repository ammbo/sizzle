...
When FC_HANDLER is login_worker.handler, WebSocket upgrades are accepted
and proxied to the interactive VNC session.
"""
...
def _is_login_handler() -> bool:
    return os.environ.get("FC_HANDLER", "").startswith("login_worker")


class Handler(BaseHTTPRequestHandler):
    _fc_handler: Callable | None = None
    _ws_handed_off: bool = False
    ...
    def handle_one_request(self):
        try:
            ...
            if _is_login_handler() and upgrade == "websocket":
    ...
    def _handle_websocket(self):
        ...
        self._ws_handed_off = True
        self.close_connection = True
        login_worker.handle_websocket(conn, path, headers)

    def finish(self):
        # Do not close the socket after handing it to the WebSocket proxy
        if self._ws_handed_off:
            return
        super().finish()
...
if __name__ == "__main__":
    ...
    print(f"FC handler ready on :{port} (login_ws={_is_login_handler()})", flush=True)
