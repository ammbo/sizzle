...
def make(
    ...
    duration: int = typer.Option(180, "--duration", help="Maximum duration in seconds (video may be shorter)"),
    work_dir: Path = typer.Option(Path("runs"), "--work-dir"),
    dry_run: bool = typer.Option(False, "--dry-run", help="No model calls; stub every lane"),
):
    """Emit a demo video (60-180s), a shot manifest, and a cost report. Zero human input."""
    cfg = Config(work_dir=work_dir, max_duration_s=duration, dry_run=dry_run)
